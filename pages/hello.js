...
export default function StudentTable() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/hello?id=${searchId}`);
      const data = await response.json();

      if (response.status === 200) {
        setStudentData(data); // Set the fetched data
      } else {
        setErrorMessage(data.error); // Display error if any
      }
    } catch (error) {
      ...
      {studentData.length > 0 && (
        <div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '20px' }}>
            ...
            <tbody>
  {studentData
    .filter((course) => course.gp !== null && course.gp !== 'N/A') // Filter out courses with no GP
    .map((course, index) => (
      <tr key={index}>
